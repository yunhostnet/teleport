...
type Rotation struct {
	...
	// automatic mode based on the provided grace period to switch
	// beetween phases.
...
// GenerateSchedule generates schedule based on the time period, splitting
// two phases scheduled time
func GenerateSchedule(clock clockwork.Clock, gracePeriod time.Duration) (*RotationSchedule, error) {
	if gracePeriod == 0 {
		return nil, trace.BadParameter("empty grace period")
	}
...
type RotationSchedule struct {
	// UpdateServers specifies time to switch to update servers
	UpdateServers time.Time `json:"update_servers,omitempty"`
	// Standby specifies time to switch to standby phase
	Standby time.Time `json:"standby,omitempty"`
}

// CheckAndSetDefaults checks and sets default values on rotation schedule
