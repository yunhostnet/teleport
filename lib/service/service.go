...
const (
	...
	// DELETE IN: 2.6.0
	// AuthServerSSHReady is generated when the Auth Server has initialized the
	// HTTP-over-SSH endpoint and is ready to start accepting connections.
	AuthServerSSHReady = "AuthServerReady"

	// AuthServerTLSReady is generated when the Auth Server has initialized the
	// TLS Mutual Auth endpoint and is ready to start accepting connections.
	AuthServerTLSReady = "AuthServerReady"
	...
	ProxyWebServerReady = "ProxyWebServer"

	// ReverseTunnelReady is generated when the proxy has initialized the
	// reverse tunnel server and is ready to start accepting connections.
	ProxyReverseTunnelReady = "ReverseTunnelReady"
...
func (process *TeleportProcess) initAuthService(authority sshca.Authority) error {
	...
	process.RegisterFunc("auth.ssh", func() error {
		...
		process.BroadcastEvent(Event{Name: AuthServerSSHReady, Payload: nil})
	...
	process.RegisterFunc("auth.tls", func() error {
		process.BroadcastEvent(Event{Name: AuthServerTLSReady, Payload: nil})
...
func (process *TeleportProcess) initSSH() error {
	...
	process.RegisterFunc("ssh.node", func() error {
		...
		// node is up and running notify anyone listening
